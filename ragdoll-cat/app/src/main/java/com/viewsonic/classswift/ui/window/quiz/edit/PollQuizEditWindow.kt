...
import com.viewsonic.classswift.databinding.WindowPollQuizEditBinding
...
class PollQuizEditWindow(val applicationContext: Context) : IWindow<WindowPollQuizEditBinding> {

    override var tag: WindowTag = WindowTag.POLL_EDIT_QUIZ
    override var size: SizeInPixels = SizeInPixels(385f.dpToPx().toInt(), 515f.dpToPx().toInt())

    override val binding: WindowPollQuizEditBinding = WindowPollQuizEditBinding.inflate(
        LayoutInflater.from(
            ContextThemeWrapper(
                applicationContext,
    ...
    private fun initUI() {
        binding.apply {
            ...
            tvVoteOption.text = if (QuizSharedUiInfo.singleOrMultipleSelectionType == SelectionOptionType.MULTIPLE)
                root.context.getString(R.string.quiz_types_poll_multiple_votes)
            else  root.context.getString(R.string.quiz_types_poll_single_vote)

            csOptionPanel.setPollUi()
        }
            ...
            buttonStartQuiz.setOnCustomClickListener(object : OnLoadingButtonStateListener {
                override fun onEnableClicked() {
                    ...
                    binding.csMessageDialog.handleQuizStartWithOngoingMission(
                        ...
                        onMaskClicked = {
                            csWindowManager.bringWindowToTop(windowTag = this@PollQuizEditWindow.tag)
                        }
                    )
                }
            })
                    ...
                    override fun changeStatus(isMultipleAnswer: Boolean) {
                        binding.apply {
                            if (isMultipleAnswer) {
                                ...
                                tvVoteOption.text = root.context.getString(R.string.quiz_types_poll_multiple_votes)
                            } else {
                                QuizSharedUiInfo.singleOrMultipleSelectionType = SelectionOptionType.SINGLE
                                tvVoteOption.text = root.context.getString(R.string.quiz_types_poll_single_vote)
                            }
                        }
                        QuizSharedUiInfo.setQuizTypeByTag(tag)
                    }
                }
            csOptionPanel.setOnChangeSingleMultipleAnswerStatusListener(
    ...
    private fun initCollection() {
        coroutineScope.launch(Dispatchers.IO) {
            quizEditWindowModel.imageUploadSharedFlow.collect { isUploadSuccess ->
                Timber.d("[PollQuizEditWindow] isUploadSuccess: $isUploadSuccess")
        ...
        coroutineScope.launch(Dispatchers.IO) {
            quizCommonWindowModel.networkAvailabilityState.collect { hasNetwork ->
                Timber.d("[PollQuizEditWindow] NetworkAvailabilityState: $hasNetwork")
                withContext(Dispatchers.Main) {
    ...
    private fun startCaptureScreenshot() {
        screenshotManager.startCaptureScreenshot(
            ...
            onSuccess = {
                ...
                QuizSharedUiInfo.setQuizTypeByTag(tag)
                csWindowManager.createWindow(get(PollQuizEditWindow::class.java), Gravity.CENTER)
            },
            onFailed = {
    ...
    private fun startQuizWindow() {
        with(csWindowManager) {
            ...
            createWindow(get(PollQuizStartWindow::class.java), Gravity.CENTER)
        }
    }
