...
import com.viewsonic.classswift.databinding.WindowTextShortAnswerStartBinding
...
import com.viewsonic.classswift.ui.widget.quiz.ShortAnswerResultDetailWidget
...
class TextShortAnswerStartWindow(
    private val applicationContext: Context
) : IWindow<WindowTextShortAnswerStartBinding>, RandomDrawWidget.RandomDrawViewListener, QuizAnswerResultAdapter.OnItemInteractionListener, ShortAnswerResultDetailWidget.OnInteractionListener {
    ...
    private val shortAnswerWindowModel: ShortAnswerWindowModel by inject(ShortAnswerWindowModel::class.java)
    ...
    private val studentQuizAnswerResultAdapter: QuizAnswerResultAdapter = QuizAnswerResultAdapter(this@TextShortAnswerStartWindow)

    override var tag: WindowTag = WindowTag.WINDOW_TEXT_SHORT_ANSWER_START_QUIZ
    override var size: SizeInPixels = SizeInPixels(955f.dpToPx().toInt(), 515f.dpToPx().toInt())

    override val binding: WindowTextShortAnswerStartBinding =
        WindowTextShortAnswerStartBinding.inflate(
            LayoutInflater.from(
                ContextThemeWrapper(
                    applicationContext,
    ...
    // region [Override] - QuizAnswerResultAdapter.OnItemInteractionListener
    override fun onItemClicked(info: QuizAnswerResultInfo, position: Int) {
        if (info.answerResultState == AnswerResultState.ANSWERED) {
            with(binding) {
                widgetShortAnswerResultDetail.visibility = View.VISIBLE
                widgetShortAnswerResultDetail.switchToPositionByInfo(info)
            }
        }
    }
    // endregion

    // region [Override] - ShortAnswerResultDetailWidget.OnInteractionListener
    override fun onEyeToggleButtonClicked(isChecked: Boolean) {
        binding.tbEye.isChecked = isChecked
    }

    override fun onClosedButtonClicked() {
        binding.widgetShortAnswerResultDetail.visibility = View.GONE
    }
    // endregion

    @SuppressLint("ClickableViewAccessibility")
    private fun initUI() {
        binding.root.outlineProvider = viewOutlineProvider
        binding.root.clipToOutline = true

        binding.widgetShortAnswerResultDetail.setOnInteractionListener(this@TextShortAnswerStartWindow)
        ...
        when (QuizSharedUiInfo.quizContent.isLatexContent()) {
            true -> {
                binding.tvQuizContent.isVisible = false
                binding.cskvQuizContent.isVisible = true
                binding.cskvQuizContent.setText(QuizSharedUiInfo.quizContent)
            }
            false -> {
                binding.tvQuizContent.isVisible = true
                binding.cskvQuizContent.isVisible = false
                binding.tvQuizContent.text = QuizSharedUiInfo.quizContent
            }
        }
    ...
    private fun initOnClick() {
        binding.apply {
            ...
            tbEye.setOnCheckedChangeListener { _, isChecked ->
                val studentAnswerResultInfoList =  shortAnswerWindowModel.getShortAnswerResultInfos().map {
                    it.copy(isPartiallyVisible = !isChecked)
                }
                studentQuizAnswerResultAdapter.submitList(studentAnswerResultInfoList)
                widgetShortAnswerResultDetail.updateShortAnswerResultInfos(studentAnswerResultInfoList.filter {
                    it.answerResultState == AnswerResultState.ANSWERED
                })
                widgetShortAnswerResultDetail.updateIsEyeButtonChecked(isChecked)
            }
            llRandomDraw.setOnClickListener {
                widgetRandomDraw.visibility = View.VISIBLE
            }

            widgetShortAnswerResultDetail.setOnClickListener {
                coroutineScope.launch(Dispatchers.Main) {
                    csWindowManager.bringWindowToTop(tag)
                }
            }

            csndNetworkDisconnect.setOnClickListener {
                coroutineScope.launch(Dispatchers.Main) {
                    csWindowManager.bringWindowToTop(tag)
    ...
    private fun initCollection() {
        ...
        coroutineScope.launch(Dispatchers.IO) {
            quizCommonWindowModel.networkAvailabilityState.collect { hasNetwork ->
                withContext(Dispatchers.Main) {
                    binding.csndNetworkDisconnect.isVisible = !hasNetwork
                    if (!hasNetwork) {
                        dialogWindow?.dismiss()
                    }
                }
    ...
    private fun changeUiToQuizResultStage() {
        with(binding) {
            ...
            val studentAnswerResultInfoList =  shortAnswerWindowModel.getShortAnswerResultInfos()
    ...
    override fun onDestroy() {
        ...
        binding.cskvQuizContent.release()
