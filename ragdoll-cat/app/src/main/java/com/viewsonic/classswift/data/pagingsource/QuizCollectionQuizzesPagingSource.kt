...
class QuizCollectionFolderQuizzesPagingSource(
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, QuizInCollectionInfo> {
        return try {
            ...
            val response = quizCollectionApiService.getQuizzesInCollectionFolder(
                ...
                page = page,
                perPage = pageSize,
                quizType = "TRUE_FALSE,SINGLE_SELECT,MULTIPLE_SELECT,RECORD,SHORT_ANSWER,SINGLE_POLL,MULTIPLE_POLL")

            when (response) {
                is ApiResponse.Success-> {
                    LoadResult.Page(
                        data = response.data.quizDataList.map { quizData -> QuizInCollectionInfo(
                            quizData = quizData,
                        ...
                        // Mark end-of-pagination as soon as we know — avoids Paging 3 firing an
                        // unnecessary append.Loading round-trip that briefly flashes the footer
                        // loader. Two signals indicate "this is the last page": (1) server's
                        // meta.totalPages says so, or (2) returned count is less than requested
                        // pageSize (partial / final page).
                        nextKey = when {
                            response.data.quizDataList.isEmpty() -> null
                            page >= response.data.metaData.totalPages -> null
                            response.data.quizDataList.size < pageSize -> null
                            else -> page + 1
                        }
                    )
                }
