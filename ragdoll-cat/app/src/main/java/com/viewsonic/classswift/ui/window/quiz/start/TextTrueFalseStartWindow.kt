...
import com.viewsonic.classswift.databinding.WindowTextTrueFalseStartBinding
...
class TextTrueFalseStartWindow(
    private val context: Context
) : IWindow<WindowTextTrueFalseStartBinding>, RandomDrawWidget.RandomDrawViewListener {
    ...
    override var tag: WindowTag = WindowTag.WINDOW_TEXT_TRUE_FALSE_START_QUIZ
    override var size: SizeInPixels = SizeInPixels(
        WindowManager.LayoutParams.WRAP_CONTENT,
        WindowManager.LayoutParams.WRAP_CONTENT
    )

    override val binding: WindowTextTrueFalseStartBinding = WindowTextTrueFalseStartBinding.inflate(
        LayoutInflater.from(
            ContextThemeWrapper(
                context,
    ...
    override fun onCreate() {
        Timber.d("[onCreate] QuizSharedUiInfo.isOngoing = ${QuizSharedUiInfo.isOngoing}")
        super.onCreate()
    ...
    private fun initOptionsPanel() {
        with(binding) {
            val optionValueType: OptionValueType = when (QuizSharedUiInfo.quizOptionType) {
                QuizOptionType.ALPHABET -> OptionValueType.ALPHABET
                else -> OptionValueType.NUMBER
            }
            cstqopOptionsPanel.setQuizInfo(quizType = QuizSharedUiInfo.quizType, optionValueType, QuizSharedUiInfo.quizType == QuizType.MULTIPLE_SELECT)
            cstqopOptionsPanel.setQuizOptionList(QuizSharedUiInfo.quizOptionList)
        }
    }
    ...
    private fun initOnClick() {
        binding.apply {
            ...
            cslbDiscloseAnswer.setOnCustomClickListener(object : OnLoadingButtonStateListener {
                override fun onEnableClicked() {
                    cslbDiscloseAnswer.setLoading()
                    sendDiscloseAnswer()
                }
            })
    ...
    private fun initCollection() {
        ...
        coroutineScope.launch(Dispatchers.Main) {
            quizStartWindowModel.quizzingUiState.mapAndCollect({quizState}) { quizState ->
                binding.cstqopOptionsPanel.updateQuizState(quizState)
            }
        }
        coroutineScope.launch(Dispatchers.IO) {
    ...
    private fun setDisClosedErrorHandler() {
        coroutineScope.launch(Dispatchers.IO) {
            quizStartWindowModel.discloseAnswerErrorFlow.collectLatest { message ->
                withContext(Dispatchers.Main) {
                    binding.cslbDiscloseAnswer.setEnable()
    ...
    private fun sendDiscloseAnswer() {
        quizStartWindowModel.discloseAnswer(binding.cstqopOptionsPanel.getSelectedOptionIdList())
    }

    private fun changeUiToDiscloseAnswerStage() {
        binding.csStopwatch.stopStopwatch()
        binding.csStopwatch.isVisible = false
        binding.cslbEndQuiz.setEnable()
        binding.cslbEndQuiz.isVisible = false
        // show DiscloseAnswer button
        binding.cslbDiscloseAnswer.setEnable()
        binding.cslbDiscloseAnswer.isVisible = true
        binding.cstqopOptionsPanel.updateQuizState(QuizState.DISCLOSE_ANSWER)
    }
    ...
    private fun changeUiToQuizResultStage(trueNumber: Int, falseNumber: Int, noAnswerNumber: Int, correctOptionId: Int) {
        binding.apply {
            ...
            binding.cstqopOptionsPanel.updateAnswerOptionIds(listOf(correctOptionId))
            binding.cstqopOptionsPanel.updateQuizState(QuizState.QUIZ_RESULTS)
            cslbDiscloseAnswer.isVisible = false
            rvStudentQuizzingList.isVisible = false
            csStopwatch.isVisible = false
    ...
    override fun onDestroy() {
        ...
        binding.cstqopOptionsPanel.release()
