...
import com.viewsonic.classswift.databinding.WindowMvbCropImageBinding
...
class MvbCropImageWindow(context: Context) : IWindow<WindowMvbCropImageBinding> {
    ...
    override var tag: WindowTag = WindowTag.MVB_CROP_IMAGE
    override var size: SizeInPixels =
        SizeInPixels(LayoutParams.MATCH_PARENT, LayoutParams.MATCH_PARENT)

    override val binding: WindowMvbCropImageBinding = WindowMvbCropImageBinding.inflate(
        LayoutInflater.from(context)
    ).apply {
        //cancel button
        buttonCancelScreenshot.setOnClickListener {
            Timber.d("[buttonCancel]: clicked")
            CSWindowManager.removeWindow(WindowTag.MVB_CROP_IMAGE)
            ...
            onCropEvent = { event ->
                when (event.action) {
                    MotionEvent.ACTION_DOWN -> {
                        clToolbarScreenshot.visibility = View.INVISIBLE
                    }
                }
            }
        }
