...
import com.viewsonic.classswift.databinding.WindowMvbTextShortAnswerStartBinding
...
class MvbTextShortAnswerStartWindow(
    private val context: Context,
) : IWindow<WindowMvbTextShortAnswerStartBinding> {
    ...
    override val binding: WindowMvbTextShortAnswerStartBinding = WindowMvbTextShortAnswerStartBinding.inflate(
        LayoutInflater.from(
            ContextThemeWrapper(
                context,
    ...
    override fun onCreate() {
        Timber.d("[onCreate] MvbTextShortAnswerStartWindow")
        super.onCreate()
    ...
    private fun startStopwatch() {
        ...
        stopwatchJob = coroutineScope.startTimerInMilliSec(
            ...
            withContext(Dispatchers.Main) {
                binding.panelQuizzing.tvStopwatch.text = String.format(Locale.US, "%02d:%02d", minutes, seconds)
            }
        }
    }
    ...
    private fun populateResultBars(container: LinearLayout, count: Int, onClick: (Int) -> Unit) {
        ...
        repeat(count) { index ->
            val bar = CSResultOptionBarItem(context).apply { onBarClick = onClick }
                ...
                withContext(Dispatchers.Main) {
                    ...
                    if (uiState.quizState == QuizState.QUIZ_RESULTS) {
                        ...
                        val infos = uiState.studentQuizzingInfoList.map { student ->
                            QuizAnsweringInfo.fromStudentShortAnswerQuizzingInfo(student, toAnsweringState(student))
                        }
    ...
    private fun applyPanelVisibility(state: QuizState) {
        ...
        if (!isResult) dismissStudentAnswerPopup()
        with(binding.panelQuizzing) {
    ...
    private fun toAnsweringState(student: StudentQuizzingInfo): AnsweringState {
    ...
    private fun computeResultInfos(uiState: QuizzingUiState): List<QuizAnsweringInfo> =
    ...
    private fun bindResultData(uiState: QuizzingUiState) {
        val infos = computeResultInfos(uiState)
    ...
    private fun bindLeftBars(infos: List<QuizAnsweringInfo>) {
        ...
        bindLeftBars(submitted, notSubmitted, submitted + notSubmitted)
    }
    ...
    override fun onDestroy() {
        Timber.d("[onDestroy] MvbTextShortAnswerStartWindow")
        super.onDestroy()
