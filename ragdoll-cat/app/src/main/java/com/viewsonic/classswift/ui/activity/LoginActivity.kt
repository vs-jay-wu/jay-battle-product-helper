...
import com.viewsonic.classswift.databinding.ActivityLoginBinding
...
class LoginActivity : AppCompatActivity() {
    ...
    companion object {
        private var instance: WeakReference<LoginActivity>? = null

        fun getInstance(): LoginActivity? = instance?.get()
    }
    ...
    private fun handleIntent(intent: Intent) {
        ...
        if (!loginViewModel.isMyViewBoardBound() && ClassSwiftService.isServiceStarted()) {
            Timber.d("[B][handleIntent] : service already started in default floating mode, close LoginActivity")
            finish()
            return
        }
        when (intent.action) {
    ...
    private fun handleDeepLinkIntent(appLinkData: Uri?) {
        ...
        lifecycleScope.launch(Dispatchers.IO) {
            ...
            Timber.tag("loginFlow").d("LoginActivity -> OTA is check: ${appUpdateManager.isChecked}")
        ...
        lifecycleScope.launch {
            repeatOnLifecycle(Lifecycle.State.CREATED) {
                ClassSwiftService.isServiceStartedFlow.collect { isServiceStarted ->
                    if (isServiceStarted) {
                        this@LoginActivity.bindService(serviceIntent, connection, BIND_AUTO_CREATE)
                    }
                }
            }
        }
