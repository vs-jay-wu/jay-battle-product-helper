...
class SubmitQuizListAnswerSocketMessage(
    @Json(name = "quiz_id")
    val quizId: String = "",
    @Json(name = "lesson_id")
    val lessonId: String = "",
    ...
    val answerData: List<AnswerData> = emptyList()
) {
    companion object {
        private val moshiProvider: MoshiProvider by inject(MoshiProvider::class.java)

        fun fromJSONObject(jsonObject: JSONObject): SubmitQuizListAnswerSocketMessage {
            return moshiProvider.fromJson(SubmitQuizListAnswerSocketMessage::class.java, jsonObject.toString(), true)
        }
    }
}