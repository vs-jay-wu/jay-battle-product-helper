...
import com.viewsonic.classswift.databinding.WindowTextMultipleChoiceStartBinding
...
class TextMultipleChoiceStartWindow(
    private val context: Context
) : IWindow<WindowTextMultipleChoiceStartBinding>, RandomDrawWidget.RandomDrawViewListener {
    ...
    override var tag: WindowTag = WindowTag.WINDOW_TEXT_MULTIPLE_CHOICE_START_QUIZ
    override var size: SizeInPixels = SizeInPixels(955f.dpToPx().toInt(), 515f.dpToPx().toInt())

    override val binding: WindowTextMultipleChoiceStartBinding =
        WindowTextMultipleChoiceStartBinding.inflate(
            LayoutInflater.from(
                ContextThemeWrapper(
                    context,
    ...
    private fun discloseAnswer() {
        val correctAnswer = binding.cstqopOptionsPanel.getSelectedOptionIdList()
        if (correctAnswer.isNotEmpty()) {
            binding.cslbDiscloseAnswer.setLoading()
    ...
    private fun changeUiToDiscloseAnswerStage() {
        with(binding) {
            ...
            cslbDiscloseAnswer.setEnable()
            cslbDiscloseAnswer.visibility = View.VISIBLE
    ...
    private fun changeUiToQuizResultStage() {
        with(binding) {
            ...
            binding.cstqopOptionsPanel.updateAnswerOptionIds(quizStartWindowModel.quizzingUiState.value.discloseAnswerData.map { it.optionId }.toList())
