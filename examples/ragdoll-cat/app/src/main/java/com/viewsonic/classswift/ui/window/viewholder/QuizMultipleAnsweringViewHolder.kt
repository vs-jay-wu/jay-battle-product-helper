...
class QuizMultipleAnsweringViewHolder(
    private val binding: ItemQuizMultipleAnsweringBinding,
    ...
    fun onBind(info: QuizAnsweringInfo) {
        with(binding) {
            mcvRoot.setOnClickListener {
                Timber.d("QuizMultipleAnsweringViewHolder is clicked.")
            ...
            when (info.answeringState) {
                AnsweringState.ANSWERED -> {
                    ...
                    if (info.canShowAnswer) {
                        // 設定 MultipleAnswer Icons
                        csMultipleAnswerIcon.setAnswerOptions(info.answerOption, info.quizOptionType)
                    }
                    csMultipleAnswerIcon.isVisible = info.canShowAnswer
                ...
                AnsweringState.NOT_ANSWER -> {
                    ...
                    tvAbsent.isVisible = false
                    csMultipleAnswerIcon.isVisible = false
