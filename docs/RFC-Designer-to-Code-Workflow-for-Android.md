...
```
現況 flow(角色:designer / developer)

  [designer]   構想(腦中) ──▶ Figma 設計稿 ──▶ 口頭交付 handoff
                            (靜態,只能模擬)              │
                                                          ▼
  [developer]  production code ◀────────────── developer 實作
               (最終產品)                      (翻譯成 code)

  ↻ 來回釐清迴圈(跨人、慢、lossy):
     developer 實作遇到空白 → 回頭找 designer 釐清 → 再改稿 → 再實作
```

**提案**:Figma 定案後 → 直接落成 production,迭代收進 designer + AI 的快速內迴圈;產出即 production,developer 退成一次性鋪路 + 末端 review。

```
提案 flow(Figma 定案後 → production)

  [designer + AI]   (起點:Figma 視覺定案)
     ┌──────────── ↻ 不滿意就再講(快、同一人)────────────┐
     ▼                                                      │
  自然語描述 ──▶ AI 生 Compose UI ──▶ 即時預覽(Hot Reload)─┘
   (可丟定案稿當輸入)  (skill/hooks 生成時約束)
                              │ 滿意後
                              ▼
  開 PR ──▶ 護欄 gate(screenshot + review)──▶ merge → production
                                                      (產出即產品)

  [developer] 一次性鋪路(common module / desktop target / hooks)+ 在護欄做 review
```
...
```
描述 → AI 生 → 即時 preview ──滿意──→ PR → 護欄(screenshot + review)→ merge
                  ↑________不滿意,繼續用自然語調整________|
```
...
```
            :fixtures (common)
           ↑     ↑          ↑
     :preview  screenshot   :demo flavor
```
